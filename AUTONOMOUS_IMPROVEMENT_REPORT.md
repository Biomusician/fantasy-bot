...
- ~~**"Rosterable" still means pool-wide percentile at a few call sites**~~ Fixed in round 4: `identify_buy_low`'s eligibility filter/sort and `identify_depth_needs` now use `_need_percentile` (within-position for dynasty), the same metric `identify_needs` and `identify_drop_candidates` already used. Two remaining pool-wide `MIN_ROSTERABLE_PERCENTILE` sites (inside `_untouchable_ids`'s starter-slot protection and the opponent-fit primitives `_weakest_rosterable_percentile`/`_piece_fits`) were deliberately left alone — they're load-bearing for the self-reference-bug-prone fit-checking chain that's already had 4 review passes, and the marginal benefit didn't justify the added regression risk this late in the session.
